...
{
    public static void ProcessTexture2(Texture2D origin, out Texture2D terrainTex, out Texture2D alphaTestTex)
	{
    ...
    public static void ProcessTexture(Texture2D origin, out Texture2D alphaTestTex)
    {
        if (origin.width != origin.height)
            throw new System.Exception(string.Format("texture {0} is not square", origin.name));
        int textureSize = origin.width;
